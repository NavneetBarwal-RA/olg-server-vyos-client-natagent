...
TC-LOGGING-004
Type: Positive
Title: Debug with payload flag logs payload
Summary:
Runs configure with explicit payload debug logging enabled. The service
should emit the raw payload only when the injected debug config enables
payload logging.

Validates:
  - debug payload logging is explicit
  - payload_json field is present
  - raw payload value is present
*/
func TestLoggingDebugWithPayloadFlagLogsPayload(t *testing.T) {
	...
	assertLogContains(t, fixture.logs, "payload_json")
	assertLogContains(t, fixture.logs, secret)
}
